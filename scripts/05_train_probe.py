...
if __name__ == "__main__":
    ...
    for model_name, path, rep_key in [
        ...
        with open(f"results/{model_name}_binary_probe.json", "w") as f:
        ...
        np.save(f"results/{model_name}_binary_matrix.npy", matrix)
        np.save(f"results/{model_name}_binary_pvalues.npy", p_values)
        ...
        with open(f"results/{model_name}_progress_probe.json", "w") as f:
    ...
    if comparison and comparison.get("n_pairs", 0) > 0:
        ...
        with open("results/statistical_comparison.json", "w") as f:
            json.dump(comparison, f, indent=2)
    else:
        print("\n  Insufficient data for statistical comparison")
    
    print("\n\nAll probes complete. Results in results/")