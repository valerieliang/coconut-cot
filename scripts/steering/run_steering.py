...
def construct_contrast_vector_from_embeddings(
    ...
    if random_control:
        ...
        original_norm = torch.norm(direction).item()
        direction = direction / (original_norm + 1e-8)  # Normalize to unit norm
        direction = direction * random_scale
        print(f"  Random control vector: original_norm={original_norm:.3f}, final_norm={random_scale}")
        if random_scale <= 0.5:
            print(f"    (scale={random_scale}) - should cause minimal flips")
        else:
            print(f"    (scale={random_scale}) - WARNING: large scale may cause spurious flips")
    else:
...
def construct_per_step_steering_vectors(
    ...
    for step in range(n_hops):
        if random_control:
            # For random control, create properly normalized random vectors
            dummy_vec = torch.randn(768)  # GPT-2 hidden size
            original_norm = torch.norm(dummy_vec).item()
            vector = dummy_vec / (original_norm + 1e-8)  # Normalize to unit norm
            vector = vector * random_scale
            step_vectors[step] = vector
            if verbose:
                print(f"    Step {step}: Random vector (original_norm={original_norm:.3f}, final_norm={random_scale})")
        else:
...
def run_coconut_experiment(
    ...
    # Use per-step vectors if requested
...
def run_verbal_experiment(
    ...
    # Use args.alphas and pass verbose parameter
    sweep_results = verbal_sweep(
        ...
        sweep_dim=args.sweep_dim,
        verbose=args.verbose  # Now verbal_sweep accepts this
    )
...
def main():
        ...
        for idx in tqdm(range(len(df_selected)), desc=f"{model_type}"):
            ...
            try:
                ...
                if result:
                    ...
                    # Check if steering propagated and answer flipped
                    ...
                    for config_name, config_results in result.items():
                        ...
                        # Check metadata for propagation
                        if 'metadata' in config_results:
                            if config_results['metadata'].get('steering_propagated', False):
                                propagation_check = True
                        
                        # Check each alpha for answer flips
    ...
    for model_type in models_loaded.keys():
        ...
        # Analyze steering effects - FIXED: Count problems with flips, not total flips
        answer_flipped_problems = 0
        total_steered = 0
        propagated_count = 0
        max_effect = 0.0
        ...
        print(f"    Answer flipped: {answer_flipped_problems}/{len(model_results)} problems ({100*answer_flipped_problems/len(model_results):.1f}%)")
        if total_steered > 0:
