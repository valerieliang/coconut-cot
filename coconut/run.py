...
THERMAL_LIMIT   = 83   # °C  – begin cooldown above this
COOLDOWN_TARGET = 72   # °C  – wait until GPU drops below this
...
def thermal_pause(label: str = "") -> None:
    """
    If the GPU is above THERMAL_LIMIT, flush CUDA, free cache, then sleep
    in POLL_INTERVAL-second increments until it cools to COOLDOWN_TARGET.
    Safe to call from any rank; only rank-0 prints.
    """
    temp = gpu_temp()
    if temp is None or temp <= THERMAL_LIMIT:
        return
    ...
    if rank == 0:
        print(f"\n[thermal] {label} GPU={temp:.0f}°C > {THERMAL_LIMIT}°C  cooling …", flush=True)

    while True:
        time.sleep(POLL_INTERVAL)
        temp = gpu_temp()
        if temp is None or temp <= COOLDOWN_TARGET:
            break
        if rank == 0:
            print(f"[thermal] still {temp:.0f}°C, waiting …", flush=True)

    if rank == 0:
        print(f"[thermal] resumed  GPU={gpu_temp():.0f}°C", flush=True)
...
def main():
    ...
    for epoch in range(configs.resume, configs.num_epochs):
        ...
        if not configs.only_eval:
            ...
            # ── thermal break after training loop ────────────────────────────
            thermal_pause(f"epoch {epoch+1} train done")
            # ─────────────────────────────────────────────────────────────────
            ...
            # ── thermal break after checkpoint save ──────────────────────────
            thermal_pause(f"epoch {epoch+1} checkpoint saved")
            # ─────────────────────────────────────────────────────────────────
        ...
        # ── thermal break at end of full epoch (train + eval) ────────────────
        thermal_pause(f"epoch {epoch+1} complete")
        # ─────────────────────────────────────────────────────────────────────
